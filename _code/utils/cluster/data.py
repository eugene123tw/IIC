...
from _code.datasets.clustering.truncated_dataset import TruncatedDataset
from _code.utils.cluster.transforms import sobel_make_transforms, \
  greyscale_make_transforms
from _code.utils.semisup.dataset import TenCropAndFinish
...
def _create_dataloaders(config, dataset_class, tf1, tf2,
                        ...
                        target_transform=None,
                        shuffle=False):
  """ Creating auxiliary dataloader base on `num_dataloaders`

  Args:
    config (argparse.Namespace): arguments
    dataset_class (torchvision.datasets())): Torch vision dataset
    tf1 (torchvision.Compose):
    tf2 (torchvision.Compose):
    partitions (list): training set if element is True, testing set otherwise
    target_transform :
    shuffle (bool):
  ...
  train_imgs = ConcatDataset(train_imgs_list)  # Concat both train and test
